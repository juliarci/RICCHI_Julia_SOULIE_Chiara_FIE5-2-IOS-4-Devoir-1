...
df3 <- df2 %>%
  transmute(ID, TYPE, Activity_DETAILS,
            t0 = Timestamp_start,
            t1 = `début op1`,
            t2 = `fin op1`,
            t3 = `début op2`,
...
# Replace old activity names with new names
df3$Activity_DETAILS <- factor(
  df3$Activity_DETAILS,
  levels = ancien,
  labels = nouveau
)
df3
...
# Verify the result
unique(df3$Activity_DETAILS)

# Arrange timestamps ascending
df4 <- df3 %>%
  arrange(time)
df4
...
df5 <- df4 %>%
  mutate(
    CURRENT_OPERATION_NUMBER = case_when(
      str_detect(type, "t1|t2") ~ 1,  # début ou fin op1
      str_detect(type, "t3|t4") ~ 2,  # début ou fin op2
      str_detect(type, "t5|t6") ~ 3,  # début ou fin op3
      str_detect(type, "t7|t8") ~ 4,  # début ou fin op4
      TRUE ~ NA_integer_
    )
  )
df5
...
# Ensure character type for ID and WITH columns
final_df <- final_df %>%
  mutate(ID = as.character(ID),
         WITH_1 = as.character(WITH_1),
         WITH_2 = as.character(WITH_2))

# Create trace_full including ACTORS derived from WITH_1 and WITH_2
trace_full <- final_df %>%
  filter(!is.na(WITH_1)) %>%
  mutate(
    WITH_2 = NA,               
  ...
  select(ID, TYPE, CURRENT_LOCATION, NEXT_LOCATION, CURRENT_OPERATION_NUMBER, WITH_1, WITH_2, DATETIME_BEGIN, DATETIME_END) %>%
  mutate(TYPE = "ACTOR") %>%
  bind_rows(
    final_df %>%
      filter(!is.na(WITH_2)) %>%
...
# Inspect only the non-patient rows
# trace_full %>%
#   filter(TYPE != "PATIENT")
```
...
df_original <- read_csv2("local_data/trace_example_final.csv")

df_patients <- df_original %>%
  filter(TYPE == "PATIENT")

#  Calculer la durée de chaque enregistrement (en minutes)
df_patients <- df_patients %>%
  mutate(duration_min = as.numeric(difftime(DATETIME_END, DATETIME_BEGIN, units = "mins")))

#  Calculer le temps total et la distance totale par patient
...
#  Identifier le patient avec le temps de séjour le plus long
patient_longest <- df_patient_total %>%
  filter(total_duration_min == max(total_duration_min))

print("Patient with the longest stay:")
print(patient_longest)

# ⃣ Pour constater où se situe la cause (optionnel)
# On peut afficher son parcours chronologique (toutes ses lignes)
...
df_original <- read_csv2("local_data/trace_example_final.csv")  # contient type
df_counts   <- read_excel("Log_Patient_URO_12112015.xlsx")  # contient les colonnes Activity_GENERAL/DETAILS

# Faire un join pour combiner type et activités
# Supposons que la colonne d'identifiant patient est "Patient_ID"
df_counts$ID <- as.character(df_counts$ID)
df_original$ID <- as.character(df_original$ID)

df_combined <- df_counts %>%
  left_join(df_original %>% select(ID, TYPE, DATETIME_BEGIN, DATETIME_END), by = "ID")

df_combined

# Filtrer uniquement les patients
df_patients <- df_combined %>%
  filter(TYPE == "PATIENT") %>%
  mutate(duration_min = as.numeric(difftime(DATETIME_END, DATETIME_BEGIN, units = "mins")))

# Calculer temps total, temps d'attente et temps d'activité par patient
df_time_summary <- df_patients %>%
  group_by(ID) %>%
  summarise(
    total_time = sum(duration_min, na.rm = TRUE),
    waiting_time = sum(duration_min[grepl("Attente", Activity_MACRO,  ignore.case = TRUE)], na.rm = TRUE),
    activity_time = sum(duration_min[!grepl("Attente", Activity_MACRO, ignore.case = TRUE)], na.rm = TRUE),
...
ggplot(df_time_summary, aes(x = total_time)) +
  ...
  labs(title = "Distribution du temps total de séjour des patients", x = "Temps total (min)", y = "Nombre de patients")

ggplot(df_time_summary, aes(x = waiting_time)) +
  geom_histogram(bins = 30, fill = "orange", color = "black") +
  labs(title = "Distribution du temps d'attente des patients", x = "Temps d'attente (min)", y = "Nombre de patients")
...
ggplot(df_time_summary, aes(x = total_time, y = waiting_time)) +
  ...
  labs(title = "Temps d'attente en fonction du temps total de séjour", x = "Temps total (min)", y = "Temps d'attente (min)")


```
#### Analyse

L’analyse de la distribution du temps total de séjour des patients met en évidence une forte asymétrie. La majorité des patients présente un temps de séjour inférieur à 2000 minutes, avec un pic marqué autour de 500 minutes, correspondant au fonctionnement nominal du service pour la plupart des parcours. Toutefois, la présence de quelques cas extrêmes, avec des durées de séjour atteignant près de 5000 et plus de 6000 minutes, révèle des situations atypiques susceptibles d’indiquer des phénomènes de congestion ou des dysfonctionnements ponctuels du processus. Ces cas extrêmes contribuent significativement à l’augmentation de la moyenne globale.

La distribution du temps d’attente des patients présente une forme similaire, fortement asymétrique, avec une majorité de patients subissant des temps d’attente modérés, mais une minorité exposée à des attentes très élevées. Cette dispersion importante traduit une variabilité marquée des parcours et met en évidence que, si le service fonctionne de manière acceptable pour la plupart des patients, certains parcours sont fortement pénalisés par des périodes d’attente prolongées.

L’analyse du ratio entre le temps d’attente et le temps total de séjour montre que l’attente représente une part significative du parcours patient. La médiane de ce ratio se situe autour de 40 %, avec une dispersion relativement limitée entre les patients. Ce résultat suggère que l’attente constitue une composante structurelle du fonctionnement du service, affectant de manière relativement homogène l’ensemble des patients, plutôt qu’un phénomène marginal limité à quelques cas isolés.

Enfin, l’étude de la relation entre le temps d’attente et le temps total de séjour met en évidence une corrélation quasi linéaire très forte. Cette relation indique que l’allongement des temps de séjour est principalement dû à l’augmentation des temps d’attente, et non à une augmentation du temps d’activité. Ainsi, les patients ayant les séjours les plus longs sont avant tout ceux qui subissent les attentes les plus importantes, ce qui confirme le rôle central de l’attente dans la dégradation de la performance globale du service.
...
## Niveau d'occupation à chaque instant et lissé.

# 1) Lecture des données
df <- read_csv2("local_data/trace_example_final.csv")

# 2) Conversion des dates + fuseau horaire
df <- df %>%
  mutate(
...
# 3) Construction des séjours par patient
sejours_service <- df %>%
  filter(TYPE == "PATIENT") %>%
  group_by(ID) %>%
  summarise(
...
# 4) Transformation en événements
...
## Niveau d'occupation par bloc de 2h

# Mise sur grille par bloc de 2h
...
## Niveau d'occupation moyen sur la journée
...
Le nombre maximal de patients présents simultanément atteint environ 14 à 15 patients.

L’occupation moyenne sur l’ensemble de la journée est de 9,07 patients.
